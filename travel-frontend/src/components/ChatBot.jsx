...
export default function ChatBot({ tripId, members = [] }) {
  ...
  useEffect(() => {
    chatApi.getHistory(tripId).then(r => setMessages(r.data));
  }, [tripId]);
  ...
  const sendMessage = async (text, askAI = false) => {
    ...
    setInput('');
    setMessages(prev => [...prev, { role: 'USER', content: msg, userName: user?.name || 'You', timestamp: new Date().toISOString() }]);
    setLoading(askAI);
    try {
      const { data } = await chatApi.send(tripId, msg, user?.id, user?.name);
      setMessages(prev => {
        const filtered = prev.filter(m => m.timestamp !== new Date().toISOString() || m.role !== 'USER' || m.content !== msg);
  ...
  const clearChat = async () => {
    ...
    try {
      await chatApi.clearHistory(tripId);
      setMessages([]);
    } catch {
  ...
  return (
    <div className="flex flex-col h-[600px] sm:h-[650px]">
        ...
        {messages.length === 0 && (
          <div className="text-center py-8 sm:py-10">
            ...
            <p className="text-white/40 mb-2 text-base sm:text-lg max-w-md mx-auto">
              Chat with your trip crew! Use the <span className="text-emerald-400 font-medium">Ask AI</span> button to get travel suggestions.
            </p>
            <p className="text-white/30 text-sm mb-6">AI only responds when you ask it to</p>
      ...
      <div className="p-3 sm:p-4 border-t border-white/10">
        ...
        <p className="text-white/20 text-xs mt-2 text-center">"Send" for group chat only. "Ask AI" triggers AI response.</p>
      </div>
    </div>
  );
}
